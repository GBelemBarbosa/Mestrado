function CBB(Φ:: Function, ∂f:: Function, pαₖ:: Function, x₀:: Array{<:Number}, α₀:: Number, ρ:: Number, γ:: Number, αmin:: Number, αmax:: Number, M:: Int64, L:: Int64, P:: Int64, N:: Int64, γ₁:: Number, γ₂:: Number, θ:: Number, k_max:: Int64; ϵ=eps(), p=Inf) 
    start=time()
    x_best=x_=x=x₀
    ∂fx_=∂fx=∂f(x)
    ...
    while true
        ...
        sₖTyₖ=sₖ'yₖ
        nyₖ=norm(yₖ)
        if j>=m || flag || sₖTyₖ/(sqrt(nsₖ)*nyₖ)>=θ
            if sₖTyₖ>0
                αₖ=nsₖ/sₖTyₖ
                ...
                if j>=1.5*m
                    αₖ=min(αmax, max(sqrt(nsₖ)/nyₖ, αₖ))
                end
            end
        end
    end 
