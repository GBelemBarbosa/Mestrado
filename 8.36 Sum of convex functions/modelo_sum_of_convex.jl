...
function f(x:: Vector{<:Number}, i:: Int64)
    if i==1
        fx=sum(x)
    elseif i==2
        fx=norm(x, 2)
...
function Σfᵢ(x:: Vector{<:Number}; m=m, f=f)
    sum=0.0
...
function ∂f(x:: Vector{<:Number}, i:: Int64; n=n)
    if i==1
        fx=ones(n)
    elseif i==2
        nx=norm(x, 2)
...
function Σ∂fᵢ(x:: Vector{<:Number}; m=m, n=n, ∂f=∂f) #Função objetivo
    sum=zeros(Float64, n)
...
function E∂f(x:: Vector{<:Number}; m=m, Θ=Θ, Σ∂fᵢ=Σ∂fᵢ)
    aux=Σ∂fᵢ(x)

    return (sqrt(2*Θ)/norm(aux)).*aux
end

PC(x:: Vector{<:Number})=min.(max.(x, -1), 1) #Projeção de x em C
