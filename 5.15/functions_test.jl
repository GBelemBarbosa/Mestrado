...
f(x:: Vector{Float64}, A:: Matrix{Float64}, y:: Vector{Float64}, λ:: Float64)=((A*x-y)⋅(A*x-y)+λ*x⋅x)/2
fᵣ(x:: Vector{Float64}; A=A, y=y, λ=λ)=((A*x-y)⋅(A*x-y)+λ*x⋅x)/2

g(x:: Vector{Float64})=(norm(A*x-y)^2+λ*norm(x)^2)/2
g(x:: Vector{Float64}, A:: Matrix{Float64}, y:: Vector{Float64}, λ:: Float64)=(norm(A*x-y)^2+λ*norm(x)^2)/2
...
function h(x:: Vector{Float64}, A:: Matrix{Float64}, y:: Vector{Float64}, λ:: Float64)
    aux=A*x-y
    return (aux⋅aux+λ*x⋅x)/2
end
...
function repeval(f:: Function, A:: Matrix{Float64}, y:: Vector{Float64}, λ:: Float64)
    for i in 1:10000
        res = f(randn(n), A, y, λ)
    end
end
